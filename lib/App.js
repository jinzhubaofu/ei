...
App.prototype.bootstrap = function (initialState) {
    ...
    locator
        .init(this.mode)
        .start()
        .on('redirect', u.bind(this.onLocatorRedirect, this));
    ...
    return me
        ...
        .then(function (Page) {
            ...
            // 如果提供了 initialState，那么我们认为数据已完成处理
            // 不触发 `init` 的 reducer过程，直接进入 render 过程
            // 否则我们进入 initialState 的加载过程，加载完成后进入 reducer 处理
...
App.prototype.onLocatorRedirect = function (path, query) {
    ...
    return me
        ...
        .then(function (Page) {
            ...
            // 这里判断一下当前 page 与 nextPage 是否为同一个类型
            ...
            var page = me.page instanceof Page
                ? me.page
                : new Page();
